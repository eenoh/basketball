  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center px-6 py-10">
      ...
      <div className="header-tabellen-container w-full max-w-6xl mt-6 mb-2 flex justify-start gap-4">
        {userIsCoach && (
          <>
            <button className="add-player-button" onClick={() => router.push(`/new-player/${teamId}`)}>
              + Add Player
            </button>

            <button className="edit-gym-button" onClick={() => router.push(`/edit-gym/${teamId}`)}>
              Edit Gym
            </button>
          </>
        )}

        <button className="team-stats-button" onClick={() => router.push(`/team-stats/${teamId}`)}>
          Team Stats
        </button>
          ...
          <tbody>
            {players.map((player) => (
              <tr key={player.id} className="border-t border-gray-200 text-sm">
                <td className="px-4 py-2">{playerNames[player.username] || player.username}</td>
